...
- [ ] Implement `ZapAdapter` in `cosf/engine/adapters/zap.py` with container support.
- [ ] Implement `BurpAdapter` with support for Burp Suite's REST API.
- [ ] Ensure DAST results are mapped to `Vulnerability` and `Asset` objects in SOM.
...
- [ ] Implement `PythonAdapter` to run custom user-provided Python scripts.
- [ ] Implement `ShellAdapter` (optional, for running local binary commands with caution).

## Phase 4: Verification & Documentation
- [ ] Create unit tests for each new adapter (`tests/test_zap_adapter.py`, etc.).
